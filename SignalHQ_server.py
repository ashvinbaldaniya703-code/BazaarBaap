"""
╔══════════════════════════════════════════════════════════════════════╗
║            SignalHQ — Live Market Intelligence — BACKEND v4.0        ║
║                                                                      ║
║  LIVE DATA SOURCES:                                                  ║
║   1. Dhan API        → NIFTY/BankNifty/FinNifty live LTP            ║
...
║  EXTRAS:  Gift Nifty, Market Sentiment, Tech Flow Meter             ║
║  TELEGRAM: /status /scan /pcr /fii /gift /sentiment /help           ║
║                                                                      ║
║  🔑 ALL KEYS via Environment Variables — no hardcoding!             ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
# ══════════════════════════════════════════════════════════════
#  🔑  API KEYS — ALL from Environment Variables (NEVER hardcode)
#  Render Dashboard → Environment → Add each key below
# ══════════════════════════════════════════════════════════════
...
# AngelOne API (Future use)
ANGEL_KEY        = os.getenv("ANGEL_KEY",        "")
...
# Anthropic AI (for /api/ai/* endpoints)
...
def send_telegram(msg: str):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHATID:
        print("[TG] Token/ChatID not configured — skipping")
        return
    try:
...
# ══════════════════════════════════════════════════
#  📡  SOURCE 2: YAHOO FINANCE — Real Candles
# ══════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════
#  📡  SOURCE 3: NSE OFFICIAL
#  FIX: Shared session with cookie caching
# ══════════════════════════════════════════════════
_nse_sess = None
_nse_sess_ts = 0
NSE_SESS_TTL = 300  # refresh session every 5 min

def _nse_session():
    global _nse_sess, _nse_sess_ts
    now = time.time()
    if _nse_sess is None or now - _nse_sess_ts > NSE_SESS_TTL:
        s = requests.Session()
        s.headers.update({
...
def get_nse_indices() -> dict:
    result = {}
    # FIX: Added S&P BSE SENSEX to the map
    MAP = {
        "NIFTY 50":           "nifty",
        "NIFTY BANK":         "banknifty",
        "NIFTY FIN SERVICE":  "finifty",
...
def get_stooq_data() -> dict:
    SYMS = {
        "nifty_s":    "^nsei",   "banknifty_s": "^nsebank",
        "sensex_s":   "^bsesn",  "gold_usd":    "xauusd",
        "crude_usd":  "clusd",   "silver_usd":  "xagusd",
        "usdinr":     "usdinr",
    }
    result = {}
...
def calc_macd(s, fast=12, slow=26, sig=9):
    ...
    return round(float(m.iloc[-1]), 2), round(float(sl.iloc[-1]), 2)
...
def calc_pivot(h, l, c):
    p = (h + l + c) / 3
    return {
        "pivot": round(p, 2), "r1": round(p * 2 - l, 2), "r2": round(p + (h - l), 2),
        "s1":    round(p * 2 - h, 2), "s2": round(p - (h - l), 2),
    }
...
def generate_signal(name: str, df, price: float) -> dict:
    blank = {
        "signal": "WAIT ⏳", "entry": 0, "sl": 0, "target": 0,
        "rsi": 50, "macd": 0, "macd_signal": 0, "atr": 0,
        "ema9": price, "ema20": price, "ema50": price, "ema200": price,
        "vwap": price, "bb_upper": price, "bb_lower": price,
    ...
    if bull >= 7:
        sig = "BUY CALL 🚀"; entry = price
        sl  = round(entry - ATR_MULT * atr, 2)
        tgt = round(entry + RR * (entry - sl), 2)
    elif bear >= 7:
        sig = "BUY PUT 📉";  entry = price
        sl  = round(entry + ATR_MULT * atr, 2)
        tgt = round(entry - RR * (sl - entry), 2)
    elif bull >= 5:
        sig = "WEAK BUY 🟡"; entry = price
        sl  = round(entry - ATR_MULT * atr, 2)
        tgt = round(entry + RR * (entry - sl), 2)
    else:
    ...
    return {
        "signal": sig, "entry": entry, "sl": sl, "target": tgt,
        "rsi": rsi, "macd": macd, "macd_signal": msig, "atr": atr,
        "ema9": ema9, "ema20": ema20, "ema50": ema50, "ema200": ema200,
        "vwap": vwap, "bb_upper": bb_up, "bb_mid": bb_mid, "bb_lower": bb_lo,
        "bullish_votes": bull, "bearish_votes": bear,
        "qty": qty, "pivot": piv, "adx": adx, "stoch_k": sk, "stoch_d": sd,
        "src": "REAL_CANDLES"
    }
...
def calc_market_sentiment(signals, fii_dii, oc_data, news_bull, news_bear):
    ...
    if combined > 2000:   score += 15; reasons.append(f"FII+DII ₹{combined:.0f}Cr ←🟢 Strong Buy")
    elif combined > 500:  score += 8;  reasons.append(f"FII+DII ₹{combined:.0f}Cr ←🟢 Moderate Buy")
    elif combined < -2000: score -= 15; reasons.append(f"FII+DII ₹{combined:.0f}Cr ←🔴 Heavy Sell")
    elif combined < -500:  score -= 8;  reasons.append(f"FII+DII ₹{combined:.0f}Cr ←🔴 Moderate Sell")

    pcr = oc_data.get("pcr", 1.0) if oc_data else 1.0
    if pcr > 1.3:   score += 12; reasons.append(f"PCR {pcr} ←🟢 Bullish (High Put Writing)")
    elif pcr > 1.0: score += 5;  reasons.append(f"PCR {pcr} ←🟡 Slightly Bullish")
    elif pcr < 0.7: score -= 12; reasons.append(f"PCR {pcr} ←🔴 Bearish (High Call Writing)")
    elif pcr < 1.0: score -= 5;  reasons.append(f"PCR {pcr} ←🟡 Slightly Bearish")

    nifty_sig = signals.get("nifty", {})
    bull_v = nifty_sig.get("bullish_votes", 4)
    bear_v = nifty_sig.get("bearish_votes", 4)
    if bull_v >= 7:   score += 15; reasons.append(f"Technical 🟢 {bull_v}/10 Bullish votes")
    elif bull_v >= 5: score += 7;  reasons.append(f"Technical 🟡 {bull_v}/10 Bullish votes")
    elif bear_v >= 7: score -= 15; reasons.append(f"Technical 🔴 {bear_v}/10 Bearish votes")
    elif bear_v >= 5: score -= 7;  reasons.append(f"Technical 🟡 {bear_v}/10 Bearish votes")

    rsi = nifty_sig.get("rsi", 50)
    if 50 < rsi < 70:  score += 8; reasons.append(f"RSI {rsi} ←🟢 Bullish zone")
    elif rsi >= 70:    score -= 5; reasons.append(f"RSI {rsi} ←🔴 Overbought")
    elif 30 < rsi < 50: score -= 5; reasons.append(f"RSI {rsi} ←🔴 Bearish zone")
    elif rsi <= 30:    score += 5; reasons.append(f"RSI {rsi} ←🟡 Oversold bounce possible")

    total_news  = news_bull + news_bear + 1
    news_bull_pct = news_bull / total_news * 100
    if news_bull_pct > 60:               score += 8; reasons.append(f"News 📰 {news_bull_pct:.0f}% Bullish")
    elif news_bull_pct < 40 and news_bear > 2: score -= 8; reasons.append(f"News 📰 {100-news_bull_pct:.0f}% Bearish")

    adx = nifty_sig.get("adx", 25)
    if adx > 35: reasons.append(f"ADX {adx} ←💪 Strong Trend")
    elif adx < 20: reasons.append(f"ADX {adx} ←⚠️ Weak/No Trend")
    ...
    return {
        ...
        "direction": direction, "direction_confidence": direction_pct,
        "reasons": reasons[:5], "pcr": pcr,
        "fii_net": fii_net,
    ...
    for idx_name, sig in signals.items():
        ...
        indicators = [
            {"name": "BB",     "val": round((price-bb_lo)/(bb_up-bb_lo+0.001)*100,1) if bb_up>bb_lo else 50,
             "bull": price > bb_mid, "label": f"BB {'↑' if price>bb_mid else '↓'}"},
        ...
        flow_scores[idx_name] = {
            "score":      score, "label":  label, "color": col,
            "bull_count": bull_count, "total": len(indicators),
...
# ══════════════════════════════════════════════════
#  🤖  ANTHROPIC AI PROXY HELPER
# ══════════════════════════════════════════════════
def _call_anthropic(prompt: str, system: str = "", max_tokens: int = 800) -> str:
    """
    Proxy Anthropic API call from backend.
    Frontend never sees the ANTHROPIC_KEY.
    """
    if not ANTHROPIC_KEY:
        return "AI service not configured. Please set ANTHROPIC_KEY in Render environment variables."
    try:
...
def get_all_data(force=False) -> dict:
    ...
    # FIX: FinNifty fallback → BankNifty proxy (not Nifty) — closer in behavior
    fn_p = dhan.get("finnifty", {}).get("price") or get_yahoo_price("^NSEBANK")

    # 2. Real intraday candles (Yahoo Finance)
    n_df   = _get_candles("^NSEI")
    bn_df  = _get_candles("^NSEBANK")
    # FIX: Sensex candles from Yahoo
    sx_df  = _get_candles("^BSESN")
    ...
    if not fn_p and bn_df is not None: fn_p = round(float(bn_df["Close"].iloc[-1]), 2)
    n_p  = n_p  or 24200.0
    bn_p = bn_p or 52340.0
    fn_p = fn_p or 23500.0
    ...
    # FIX: Added Sensex signal generation
    n_sig  = generate_signal("NIFTY",      inject(n_df,  n_p),  n_p)
    bn_sig = generate_signal("BANKNIFTY",  inject(bn_df, bn_p), bn_p)
    sx_p   = (dhan.get("sensex",{}).get("price") or
              get_yahoo_price("^BSESN") or
              (round(float(sx_df["Close"].iloc[-1]),2) if sx_df is not None else 0) or 79500.0)
    sx_sig = generate_signal("SENSEX",     inject(sx_df, sx_p), sx_p)
    ...
    # 7. Sentiment + Flow
    signals_dict = {"nifty": n_sig, "banknifty": bn_sig, "sensex": sx_sig}
    ...
    _cache = {
        ...
        "indices": {
            ...
            "sensex":     merge_idx("sensex",   "sensex_s",   sx_p, "NSE"),
    ...
    _cache_ts = time.time()
    print(f"✅ DONE | N:{n_p} | SX:{sx_p} | Sig:{n_sig['signal']} | Sentiment:{sentiment['label']}({sentiment['score']})")
    return _cache
...
def _check_alert(name: str, sig: dict):
    ...
    risk   = round(abs(e - sl)  * qty, 2)
    profit = round(abs(tgt - e) * qty, 2)
    piv    = sig.get("pivot", {})
    is_buy = "BUY CALL" in new or "WEAK" in new
    msg = (
        ...
        f"⚠️ _SignalHQ — Educational only_"
    )
    send_telegram(msg)
    _last_sig[name] = new
    ...
    try:
        ...
        for upd in r.json().get("result", []):
            ...
            if text == "/status":
                ...
                ssx = _cache["signals"].get("sensex", {})
                send_telegram(f"NIFTY: *{sn['signal']}* RSI:{sn['rsi']}\nBN: *{sbn['signal']}*\nSENSEX: *{ssx.get('signal','--')}*")
            elif text == "/pcr":
                oc = get_option_chain("NIFTY")
                if oc:
                    pcr  = oc["pcr"]
                    zone = "Bullish🟢" if pcr > 1.2 else "Bearish🔴" if pcr < 0.8 else "Neutral🟡"
                ...
                send_telegram("📋 SignalHQ Commands:\n/status /scan /pcr /fii /gift /sentiment /help")
    except Exception as e:
        print(f"[CMD] {e}")
...
@app.route("/")
def root():
    return jsonify({"app": "SignalHQ", "version": "4.0", "status": "LIVE",
...
# Keep-alive endpoint — frontend pings this on load to wake Render free instance
@app.route("/wake")
def wake():
    return jsonify({"awake": True, "ts": ist_now().isoformat()})
...
@app.route("/api/news")
def api_news():
    arts, sent, b, br = get_news()
    return jsonify({"articles": arts, "sentiment": sent, "bull": b, "bear": br})
...
# ── AI Proxy endpoints (FIX: Anthropic called from backend, never frontend) ──

@app.route("/api/ai/news", methods=["POST"])
def api_ai_news():
    """Analyze a single news headline via Anthropic"""
    ...
    system = (
        "You are SignalHQ, an Indian stock market AI. "
        "Analyze news headlines for NSE/BSE impact. "
        "Reply ONLY with a JSON object: "
        "{\"sentiment\":\"bullish|bearish|neutral\","
        "\"impact\":\"high|medium|low\","
        "\"indices\":[\"NIFTY\",\"BANKNIFTY\"],"
        "\"reason\":\"one sentence in Gujarati + English terms\","
        "\"recommendation\":\"one-line trading idea\"}"
        "No extra text, no markdown."
    )
    prompt = f'Analyze: "{title}" (Source: {source})'
    result = _call_anthropic(prompt, system=system, max_tokens=400)
...
@app.route("/api/ai/fii", methods=["POST"])
def api_ai_fii():
    """FII/DII analysis in Gujarati via Anthropic"""
    body    = request.get_json() or {}
    fii_net = body.get("fii_net", 0)
    dii_net = body.get("dii_net", 0)

    prompt = (
        f"SignalHQ — FII/DII Gujarati analysis:\n"
        f"FII Net: {'+' if fii_net>=0 else ''}₹{fii_net:.0f} Cr\n"
        f"DII Net: {'+' if dii_net>=0 else ''}₹{dii_net:.0f} Cr\n\n"
        f"Provide: market direction, outlook, Nifty+Sensex options recommendation, risk factors. "
        f"Gujarati with English terms."
    )
    result = _call_anthropic(prompt, max_tokens=700)

    # Also send to Telegram
    send_telegram(
        f"📊 *SignalHQ FII/DII*\n\n"
        f"FII: {'+' if fii_net>=0 else ''}₹{fii_net:.0f}Cr\n"
        f"DII: {'+' if dii_net>=0 else ''}₹{dii_net:.0f}Cr\n\n"
        f"{result[:400]}..."
    )
...
def api_ai_alerts():
    """Generate AI trading alerts in Gujarati"""
    body = request.get_json() or {}
    sigs = body.get("signals", {})
    sent = body.get("sentiment", {})
    pcr  = body.get("pcr", 1.0)

    prompt = (
        f"SignalHQ — Gujarati trading alerts:\n"
        f"Nifty:     {sigs.get('nifty',{}).get('signal','WAIT')} | RSI:{sigs.get('nifty',{}).get('rsi','--')}\n"
        f"BankNifty: {sigs.get('banknifty',{}).get('signal','WAIT')}\n"
        f"Sensex:    {sigs.get('sensex',{}).get('signal','WAIT')} | RSI:{sigs.get('sensex',{}).get('rsi','--')}\n"
        f"Sentiment: {sent.get('label','--')} ({sent.get('score',50)}/100)\n"
        f"PCR: {pcr}\n\n"
        f"Generate 3-4 alerts: Symbol, Entry, Target, SL, Reason, Risk(1-10). "
        f"Include Hero-Zero if PCR extreme. Gujarati with English terms."
    )
    result = _call_anthropic(prompt, max_tokens=800)
    send_telegram(f"🚨 *SignalHQ AI Alerts*\n\n{result[:500]}...")
    return jsonify({"alerts": result})

@app.route("/api/telegram", methods=["POST"])
def api_telegram():
    """Send a Telegram message (frontend calls this instead of Telegram directly)"""
    body = request.get_json() or {}
...
if __name__ == "__main__":
    print("╔══════════════════════════════════════════════════════╗")
    print("║         🔥  SignalHQ — Live Market Intelligence      ║")
    print("║         Backend v4.0 — All Keys via Env Vars         ║")
    print("╚══════════════════════════════════════════════════════╝")

    # Check key config
    missing = [k for k, v in {
        "TELEGRAM_TOKEN": TELEGRAM_TOKEN, "DHAN_TOKEN": DHAN_TOKEN,
        "NEWSDATA_KEY":   NEWSDATA_KEY,   "ANTHROPIC_KEY": ANTHROPIC_KEY,
    }.items() if not v]
    if missing:
    ...
    print(f"🎁 Gift:   {d['gift_nifty'].get('price','?')}")
    print(f"🎯 Signal: {sn['signal']} | RSI:{sn['rsi']}")
    print(f"📡 Sent:   {sent['label']} ({sent['score']}/100)")

    send_telegram(
        f"🔥 *SignalHQ v4.0 LIVE!*\n"
